# -*- coding: utf-8 -*-
"""
Created on Wed Apr 24 17:49:42 2024
...
# Paso 5: Iniciar ciclo de 10 iteraciones   //Al cambiar el dato al 20, grafica 20 iteraciones
for i in range(10):
    ...
    # Lista de vectores de soporte por iteración
    vectores_soporte_por_iteracion_y = [len(iteracion) for iteracion in vectores_soporte_por_iteracion]
    
    # Números de iteración en el eje X
    iteraciones_x = range(1, len(vectores_soporte_por_iteracion_y) + 1)
    ...
    # Graficar
    plt.plot(iteraciones_x, vectores_soporte_por_iteracion_y, marker='o', color='blue', linestyle='-')
    plt.xlabel('Iteraciones')
    plt.ylabel('Vectores de Soporte en Y')
    plt.title('Vectores de Soporte en Y por Iteración')
    plt.grid(True)
    plt.show()
